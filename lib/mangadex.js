...
const { param } = require("../routes/komikindoRoutes");
...
async function searchTags(genres = []) {
    try {
        const response = await axios.get(`${MANGADEX_API}/manga/tag`, headers);
        ...
        const filteredTags = tags
            .filter(tag => lowerCaseGenres.includes(tag.attributes.name.en.toLowerCase()))
            .map(tag => tag.id);
...
async function searchManga(type, query, genre = []) {
    // IGNORE type PARAMS
    try {
        const params = {
            limit: 100,
        ...
        if (genre.length > 0) {
            params.includedTags = await searchTags(genre);
        }

        const response = await axios.get(`${MANGADEX_API}/manga`, { params, headers });

        const formattedResults = await Promise.all(
            response.data.data.map(manga => formatMangaData(manga))
        );

        return { max_page: 1, results: formattedResults };
    } catch (error) {
...
// async function main() {
//     // const data = await searchTags(['fantasy', 'romance']);
//     // console.log(data);

//     // const dataS = await searchManga("manga", "", ['fantasy', 'romance']);
//     // console.log(dataS);
