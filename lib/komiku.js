...
async function searchComic(type, query) {
	const html = await axios.get(`${API_URL}/?post_type=${type}&s=${query}`, headers);
	...
	return { max_page: 1, results: mangaList};
}
...
async function getList(filter) {
	let url;
	if (filter) {
		url = `${MAIN_URL}/daftar-komik/?tipe=${filter}`;
	} else {
		url = `${MAIN_URL}/daftar-komik`;
	}
	...
	try {
		const {
			...
			retry: 3,
			retryDelay: 1000,
			headers: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:120.0) Gecko/20100101 Firefox/120.0'
		});
		const $ = cheerio.load(data);
		const result = [];
